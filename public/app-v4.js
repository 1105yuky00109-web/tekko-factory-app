...
async function resolveUserCompany(email, uid) {
    ...
    if (impCompanyId && isDeveloper) {
        showDebugLog("Impersonation mode active for companyId: " + impCompanyId);
        try {
            ...
            if (docSnap.exists()) {
                ...
                companyData.role = 'admin'; // 代理ログイン中は管理者権限を付与
                showDebugLog("Company resolved via Impersonation: " + (companyData.companyName || companyData.companyId));
                return companyData;
            }
        } catch (e) {
            console.error("Impersonation resolve error:", e);
        }
    }
