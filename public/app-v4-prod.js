...
async function resolveUserCompany(email, uid) {
    ...
    if (email && email.toLowerCase().trim() === 'steelworks@areva.co.jp') {
        try {
            const docSnap = await getDoc(doc(db, "companies", "c_78i9f5ky"));
            if (docSnap.exists()) {
                const companyData = docSnap.data();
                companyData.companyId = "c_78i9f5ky";
        ...
        try {
            ...
            if (user) {
                ...
                const isBypassEmail = user.email.includes('oowada') || 
                                      ...
                                      user.email === 'steelworks@areva.co.jp' ||
